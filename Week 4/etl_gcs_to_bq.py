...
def extract_from_gcs(color: str, year: int, month: int) -> Path:
    ...
    gcs_path = f"{color}\{color}_tripdata_{year}-{month:02}.csv"
    gcs_block = GcsBucket.load("zoom-gcs")
    gcs_block.get_directory(from_path=gcs_path, local_path=f"{color}/")
    return Path(f"{color}/{color}_tripdata_{year}-{month:02}.csv")
...
def write_bq(df: pd.DataFrame) -> int:
    ...
    gcp_credentials_block = GcpCredentials.load("zoom-gcp-student-creds")

    df.to_gbq(
        destination_table="trips_data.yellow_trips",
        project_id="de-zoomcamp-376202",
